...
def random_mach(genelist, dftrain, dftest, genenum=5):
    all_scores = []
    for i in range(1000):
...
def compare_two(pp, dfpair, training, testing, genelist1, genelist2, name1, name2):
        # run analyses:
        print "analysing gene list 1..."
        genesize1 = len(genelist1)
        all_scores1 = random_mach(genelist1, *dfpair)

        print "analysing gene list 2..."
        genesize2 = len(genelist2)
        all_scores2 = random_mach(genelist2, *dfpair, genenum=5)
        ...
        plt.title("%s\n%s (red): %d genes | %s (blue): %d genes\nJensen-Shannon divergence: %.3f" % (title, name1, genesize1, name2, genesize2,JSD(norm1, norm2)))
...
if __name__ == '__main__':
    verbalise = config.check_verbose(False)
    if len(sys.argv)<2:
    ...
    bsse_r = [ os.path.join(config_d['bsse_rd'],file)
                for file in os.listdir(config_d['bsse_rd']) if re.search("B[456].*(_FL|_SP).+htseq.gene",file) ]
    ...
    ultimate = list(set(["LOC105276751",
                        "LOC105279871",
                        "LOC105280489",
                        "LOC105279696",
                        "LOC105287193",
    ...
    combos = [((cbir_df,acro_ortho),"C.biroi PE scaled","Acromyrmex scaled",
